...
const crawlerClient = axios.create({
  baseURL: import.meta.env.VITE_EMAIL_API_URL || 'http://i9ontd9frpiurphdjkudw5mw.103.74.122.58.sslip.io/api',
...
export function getAttachmentDownloadUrl(attachmentId: number): string {
  const base = (import.meta.env.VITE_EMAIL_API_URL || 'http://i9ontd9frpiurphdjkudw5mw.103.74.122.58.sslip.io/api').replace(/\/$/, '')
...
export async function getEmailOrders(skip = 0, limit = 50): Promise<EmailOrderListItem[]> {
  const { data } = await client.get('/email-orders', { params: { skip, limit } })
  return data
}
...
export async function syncEmailToDB(crawlerEmail: CrawlerEmail): Promise<EmailOrder> {
...
export async function convertAttachment(attachmentId: number): Promise<EmailAttachment> {
  const { data } = await client.post(`/email-orders/attachments/${attachmentId}/convert`)
  return data
}

/** User confirms OCR done (processing → done). */
export async function doneAttachment(attachmentId: number): Promise<EmailAttachment> {
  const { data } = await client.post(`/email-orders/attachments/${attachmentId}/done`)
  return data
}

/** Bulk mark multiple attachments as processing. */
export async function bulkConvert(attachmentIds: number[]): Promise<EmailAttachment[]> {
